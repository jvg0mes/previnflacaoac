...
ipd_md = pd.read_csv('data/ipeadata_metadata.csv',encoding='latin-1',sep=';')
yf_md = pd.read_csv('data/yf_metadata.csv',encoding='latin-1',sep=';')


# IMPORTANDO ARQUIVOS DE SERIES
bcb_s = pd.read_csv('data/bcb_series.csv',encoding='latin-1',sep=';',index_col='Date')
...
series_list = [bcb_s,ipd_s,yf_s]


cframe = series_list[0]
...
bcb_md[np.isin(bcb_md['Codigo'].astype(str),cframe.columns)]['Unidade'].unique()


nonvar_columns = bcb_md[-bcb_md['Unidade'].str.contains('%')]['Codigo'].to_numpy()
...
fdict['frame'].to_csv('final_data/bcbframe.csv',sep=';')
